                            ...
                            <table id="tabelFormatPP" class="min-w-full border border-gray-300 text-sm">
                                ...
                                <tbody>
                                    <template x-for="(row, idx) in $store.pp.formatRows" :key="row.kode_obat">
                                        <tr :class="idx % 2 ? 'bg-gray-50' : ''">
                                            <td class="border px-2 py-1 text-center">
                                            <input type="checkbox" class="row-check"></td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 🔹 Clear → sisakan 1 row kosong
    document.getElementById('clearRowsKeluar').addEventListener('click', function () {
        const rows = tbodyKeluar.querySelectorAll('tr');
        if (rows.length === 0) return;

        const firstRow = rows[0].cloneNode(true);
        $(firstRow).find('.select2-container').remove();
        resetRowValuesKeluar(firstRow);

        tbodyKeluar.innerHTML = '';
        tbodyKeluar.appendChild(firstRow);
    ...
    formLK.addEventListener('submit', function (e) {
        ...
        .then(resp => {
            if (resp.success) {
                alert(resp.message);
                // reset form/baris
                document.getElementById('clearRowsLK').click();
                // reload seluruh halaman
                window.location.reload();
            } else if (resp.errors) {
                let messages = [];
                for (const key in resp.errors) {
                    messages.push(`${key}: ${resp.errors[key].join(", ")}`);
                }
                errorDiv.innerHTML = messages.join("<br>");
            } else if (resp.error) {
                errorDiv.innerHTML = `Error: ${resp.error}<br>File: ${resp.file}<br>Line: ${resp.line}`;
            }
        })

        .catch(err => {
            console.error("Fetch error:", err);
            // tidak usah tampilkan errorDiv lagi
        })
...
function generateNoPP() {
    ...
    if (noUrut && tglPP) {
        ...
        document.getElementById("no_pp").value =
            `${noUrut}/PP-Klinik/KSK/${bulanRomawi}/${year}`;
    } else {
        document.getElementById("no_pp").value = "";
    }
}
